...
Component({
	...
	lifetimes: {
		...
		ready: function() {
			if(!this.data.bindinfo.info||!this.data.uinfo) {
				...
				var now = new Date();
				var nowweek = (now/1000-starttime)/86400>=1&&(now/1000-starttime)/86400<=this.data.wlist.length?parseInt((now/1000-starttime)/86400):1;
				this.setData({
	...
	methods: {
		...
		changeWeek(e) {
			...
			this.setData({
				weekArrays: this.getWeekArrays(this.data.semeter,e.detail.current+1)
			})
		},
		getTimeTable(semester) {
			...
			if (data.username != "" && data.password != "") {
				...
				api.POST("https://wtbukb.miuss.mcrealms.cn/api/?act=wxcourse&semesterid="+that.data.schoolYear.id[semester], data, ).
				then(result => {
					if (result.status != "error") {
						var starttime = that.data.schoolYear.time[semester];
						var now = new Date();
						var nowweek = (now/1000-starttime)/86400>=1&&(now/1000-starttime)/86400<=result.data.length?parseInt((now/1000-starttime)/86400):1;
						wx.setStorage({
							key:"wlist",
							week: nowweek,
							semester: semester,
							data: that.conform(result.data)
						})
						that.setData({
						...
						that.getWeekArrays(semester,nowweek);
						wx.hideLoading();
						wx.showToast({
							title: "获取课表成功",
							icon: 'success',
		...
		conform(timeTable) {
			...
			for (let j = 0; j < len; j++) {
				...
				for (let i = 0; i < timeTable[j].length;) {
				    if (timeTable[j][i + temp] && timeTable[j][i].xqj == timeTable[j][i + temp].xqj && timeTable[j][i].kcmc == timeTable[j][i + temp].kcmc&&timeTable[j][i+temp].skjc-1 == timeTable[j][i+temp-1].skjc&&timeTable[j][i].room == timeTable[j][i+temp].room) {
				        temp++; //外层循环跳过重复的数据
				        skcd++; //节点数增加
				        continue;
				    }
